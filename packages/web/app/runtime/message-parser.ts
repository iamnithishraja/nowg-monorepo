import type { ActionType, NowGaiAction, NowGaiActionData, FileAction, ShellAction, SupabaseAction } from '../types/action';
import type { NowGaiArtifactData } from '../types/artifact';
...
const ARTIFACT_TAG_OPEN = '<nowgaiArtifact';
const ARTIFACT_TAG_CLOSE = '</nowgaiArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<nowgaiAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</nowgaiAction>';
const NOWGAI_QUICK_ACTIONS_OPEN = '<nowgai-quick-actions>';
const NOWGAI_QUICK_ACTIONS_CLOSE = '</nowgai-quick-actions>';


export interface ArtifactCallbackData extends NowGaiArtifactData {
  messageId: string;
}
...
interface MessageState {
  ...
  currentArtifact?: NowGaiArtifactData;
  currentAction: NowGaiActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(NOWGAI_QUICK_ACTIONS_OPEN, i)) {
        console.log('input:', input.slice(i));

        const actionsBlockEnd = input.indexOf(NOWGAI_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + NOWGAI_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          const quickActionRegex = /<nowgai-quick-action([^>]*)>([\s\S]*?)<\/nowgai-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + NOWGAI_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as NowGaiAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as NowGaiAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__nowgaiQuickAction__"',
    'data-nowgai-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__nowgaiQuickAction__\" data-nowgai-quick-action=\"true\">${buttons.join('')}</div>`;
}
