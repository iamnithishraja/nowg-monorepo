...
/**
 * Get billing summary for a conversation
 */
export async function getBillingSummary(conversationId: string): Promise<{
  success: boolean;
  data?: {
    conversationId: string;
    neonProjectId: string;
  ...
  try {
    ...
    const billingRecord = await NeonUsageBilling.findOne({
      conversationId: conversationId,
    });
    ...
    const recentUsage = billingRecord.usageRecords.slice(-24).map((r: any) => ({
      periodStart: r.periodStart,
      periodEnd: r.periodEnd,
      computeTimeSeconds: r.computeTimeSeconds,
      logicalSizeBytesHour: r.logicalSizeBytesHour,
      computeCost: r.computeCost,
    ...
    return {
      success: true,
      data: {
        conversationId: billingRecord.conversationId.toString(),
        neonProjectId: billingRecord.neonProjectId,
        status: billingRecord.status,
        totalBilledAmount: billingRecord.totalBilledAmount || 0,
        carryForwardCost: billingRecord.carryForwardCost || 0,
        lastBilledAt: billingRecord.lastBilledAt,
        totalUsageRecords: billingRecord.totalUsageRecords || 0,
        recentUsage,
      },
    };
  } catch (error) {
    console.error("[NeonBilling] Error getting billing summary:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : String(error),
    };
