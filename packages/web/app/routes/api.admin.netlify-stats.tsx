...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const netlifyDeployments = await Deployment.find({ platform: "netlify" });

    const totalDeployments = netlifyDeployments.length;
    const successfulDeployments = netlifyDeployments.filter(
      (d) => d.status === "success"
    ).length;
    const failedDeployments = netlifyDeployments.filter(
      (d) => d.status === "failed"
    ).length;
    const pendingDeployments = netlifyDeployments.filter(
      (d) => d.status === "pending"
    ).length;
    const uniqueUsers = new Set(netlifyDeployments.map((d) => d.userId)).size;

    const recentDeploymentsQuery = await Deployment.find({
      platform: "netlify",
    })
      .sort({ deployedAt: -1 })
      .skip(skip)
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch Netlify stats" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      }
