...
export type TransactionSource = "project_wallet" | "org_wallet" | "direct" | "usage_deduction";
...
export interface CreditBackValidationResult {
  valid: boolean;
  error?: string;
  details?: {
    maxAllowed: number;
 ...
 */
export function calculateTotalReceivedFromOrg(transactions: Transaction[]): number {
  return transactions
    .filter(
 ...
 */
export function getLastTransactionId(wallet: Wallet): string | null {
