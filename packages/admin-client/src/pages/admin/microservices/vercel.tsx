...
export default function VercelPage() {
  ...
  const { data: stats, isLoading } = useQuery<VercelStats>({
    queryKey: ["/api/admin/vercel-stats", currentPage],
    queryFn: async () => {
      const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5173";
      const response = await fetch(
        `${API_URL}/api/admin/vercel-stats?page=${currentPage}&limit=10`,
        {
          credentials: "include",
        }
      );
      if (!response.ok) throw new Error("Failed to fetch Vercel stats");
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Deploy and host applications with Vercel integration
          </p>
        </div>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={stats.dailyDeployments}>
                      ...
                      <YAxis />
                      <Tooltip />
                      <Bar dataKey="count" fill="#0088FE" name="Deployments" />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
