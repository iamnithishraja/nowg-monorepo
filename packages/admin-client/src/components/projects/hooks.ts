...
export function useOrgWallet(
  ...
  return useQuery<OrgWalletData>({
    ...
    queryFn: () =>
      client.get<OrgWalletData>(`/api/admin/org-wallets/${organizationId}`),
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      return client.put(`/api/admin/projects/${data.id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/projects"] });
      toast({
...
export function useAssignProjectAdmin() {
  ...
  return useMutation({
    mutationFn: async (data: { projectId: string; email: string }) => {
      return client.post(`/api/admin/projects/${data.projectId}/assign-admin`, {
        email: data.email,
      });
    },
    onSuccess: () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      toast({
        ...
        description: `Successfully transferred ${variables.amount} credits to project`,
      });
    },
    onError: (error: Error) => {
      toast({
